...
class ZenChef < Site::SimpleString
  def initialize(restaurant_id:, date_begin:, date_end:, every:, messages: nil, comment: nil)
    raise Exception, "Please provide date_begin with YYYY-MM-DD format" unless date_begin =~ /^\d\d\d\d-\d\d-\d\d$/
    raise Exception, "Please provide date_end with YYYY-MM-DD format" unless date_begin =~ /^\d\d\d\d-\d\d-\d\d$/
    raise Exception, "Please provide a proper restaurant_id" unless restaurant_id =~ /^\d+$/
